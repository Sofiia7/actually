...
// manifest.json's `key` field pins the UNPACKED extension's ID, so the Worker
// can allowlist a stable chrome-extension:// origin during development. This
// check catches that key silently drifting out of sync with
// ALLOWED_EXTENSION_ID in worker/wrangler.toml (a non-secret, committed value).
//
// It says nothing about the published ID. An earlier version of this comment
// claimed the key "reserves" the ID that CWS will assign on first publish -
// it does not, and the store now rejects any upload carrying the field at all
// ("key field is not allowed in manifest"). CWS generates its own key pair per
// item and derives the ID from that. The supported order runs the other way:
// upload a package WITHOUT `key` (scripts/package-cws.mjs strips it), read the
// item's public key from the dashboard's Package tab, and paste that in here -
// then the unpacked build inherits the published ID. Until that round trip is
// done, ALLOWED_EXTENSION_ID covers the dev ID only, and the published build
// will be CORS-refused by the Worker until the real ID is added and the Worker
// redeployed. https://developer.chrome.com/docs/extensions/reference/manifest/key
if (manifest.key) {
  ...
  if (allowed.length === 0) {
    ...
    ok(`manifest key resolves to extension ID ${id}, matches ALLOWED_EXTENSION_ID (dev ID - the CWS-assigned one is separate)`)
  } else {
