...
//
// An earlier version of this comment claimed the key "reserves" the ID that CWS
// will assign on first publish. It does not, and the store now rejects any
// upload carrying the field at all ("key field is not allowed in manifest").
// CWS generates its own key pair per item and derives the ID from that. The
// supported order runs the other way: upload a package WITHOUT `key`
// (scripts/package-cws.mjs strips it), read the item's public key from the
// dashboard's Package tab, and paste that in here - then the unpacked build
// inherits the published ID.
//
// That round trip is done as of 2026-08-26, so the key below is the store
// item's own and this check now compares the published ID, not a local
// stand-in. https://developer.chrome.com/docs/extensions/reference/manifest/key
if (manifest.key) {
